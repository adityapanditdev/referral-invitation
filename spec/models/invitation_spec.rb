...
RSpec.describe Invitation, type: :model do
  ...
  describe 'after_validation callback' do
    ...
    let(:invitation) { FactoryBot.create(:invitation, referred_by: user.id) }

    xit 'sends an invitation email if the record is valid' do
      expect(invitation.status).to eq('pending')
      invitation.valid?
    end

    xit 'does not send an invitation email if the record is invalid' do
      expect(invitation).not_to receive(:send_invitation)
      invitation.valid?
    end
  end
